...
class UserManager(BaseUserManager):
    def create_user(self, first_name, last_name, username, email, password=None):
        if not email:
            raise ValueError('you must have an email address'.title())
        if not username:
            raise ValueError('you must have an username'.title())
...
class UserProfile(models.Model):
    ...
    roll_no_and_section = models.CharField(max_length=20, blank=True, null=True)
    class_teacher = models.CharField(max_length=50, blank=True, null=True)
    pin_code = models.CharField(max_length=6, blank=True, null=True)
    father_phone = models.CharField(max_length=20, blank=True, null=True)
    mother_phone = models.CharField(max_length=6, blank=True, null=True)
